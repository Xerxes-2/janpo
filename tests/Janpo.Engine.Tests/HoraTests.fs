...
module HoraTests =
    ...
    let ``摸进和了牌后 Hora 进入合法动作集，排在打牌之前`` () =
        ...
        Assert.Equal<Action list>(
            ...
            List.truncate 1 (actionsOf (seat 0) state)
        )

        // 自摸和不看振听：Oya 第 1 巡打掉的 1z 与它无关，能不能和只看和了型。
        Assert.Contains(Action.Hora(context.Oya, context.Oya, pai "5z"), actionsOf (seat 0) state)
    ...
    let ``自摸和的来源必须是自己、牌必须是刚摸进那张`` () =
        ...
        Assert.Equal<IllegalAction>(
            HoraTileMismatch(context.Oya, seat 2, pai "5z"),
            rejected state (Action.Hora(context.Oya, seat 2, pai "5z"))
        )
    ...
    let private atTheRonAlone () = atTheRon () |> othersPass (seat 2)
    ...
    let ``他家打出和了牌时 Ron 与「过」一起进入对应座位的合法动作集`` () =
        ...
        Assert.Equal<Seat list>(seats [ 1; 2 ], responding state)

        Assert.Equal<Action list>(
            [ Action.Hora(seat 2, seat 0, pai "4p"); Action.None(seat 2) ],
    ...
    let ``振听座位的 Ron 不出现在合法动作集里，它照样是听牌的`` () =
        ...
        Assert.True(isTenpai (seat 1) state)
        Assert.Equal<Furiten>({ Permanent = true; Doujun = false }, furitenOf (seat 1) state)

        // **振听只挡荣和，不挡鸣牌**：座位 1 能吃 4p，因此照样被问，
        // 只是它的那份里没有 Ron。
        Assert.Contains(seat 1, responding state)
        Assert.DoesNotContain(Action.Hora(seat 1, seat 0, pai "4p"), actionsOf (seat 1) state)
        Assert.Contains(Action.Chi(seat 1, seat 0, pai "4p", tilesOf "5p 6p"), actionsOf (seat 1) state)

        // 它硬要宣言荣和：型成立、也有役（平和），挡住它的是振听。
        Assert.Equal<IllegalAction>(
            RonWhileFuriten(seat 1, pai "4p"),
            rejected state (Action.Hora(seat 1, seat 0, pai "4p"))
        )

        // 已经答复过的座位再提交，得到的才是「现在不轮到你」。
        Assert.Equal<IllegalAction>(
            NotYourTurn(seat 1, seats [ 2 ]),
            rejected (atTheRonAlone ()) (Action.Hora(seat 1, seat 0, pai "4p"))
        )

    [<Fact>]
    let ``荣和结束这一局，Ko 和了则不连庄`` () =
        let state = atTheRonAlone ()
        let ended, events = stepped state (Action.Hora(seat 2, seat 0, pai "4p"))
    ...
    let ``荣和的来源必须是刚打牌那家、牌必须是刚打出那张`` () =
        ...
        Assert.Equal<IllegalAction>(
            HoraTileMismatch(seat 2, seat 3, pai "4p"),
            rejected state (Action.Hora(seat 2, seat 3, pai "4p"))
        )

        Assert.Equal<IllegalAction>(
            HoraTileMismatch(seat 2, seat 0, pai "1p"),
            rejected state (Action.Hora(seat 2, seat 0, pai "1p"))
        )

    [<Fact>]
    let ``见逃得到同巡振听，自己下次摸牌时解除`` () =
        let state = atTheRonAlone ()
        let passed, events = stepped state (Action.None(seat 2))

        // 「过」自己不产出事件（mjai 的 none 是一次答复，不是既成事实）；
        // 收齐答复之后这一局接着打，这一步产出的只有下家的 tsumo。
        Assert.Equal<Event list>([ Tsumo(seat 1, pai "1p") ], events)
        Assert.False(GameState.isEnded passed)
        Assert.Equal<Furiten>({ Permanent = false; Doujun = true }, furitenOf (seat 2) passed)

        // 同巡振听到自己下次摸牌为止：摸完就解除，永久振听则始终没成立过。
        let afterDraw =
            passed |> driveUntil passive (fun state -> junmeOf (seat 2) state = 2)

        Assert.Equal<Furiten>(Furiten.none, furitenOf (seat 2) afterDraw)

    [<Fact>]
    let ``同巡振听期间，他家再打出和了牌也不进合法动作集`` () =
        let state = atTheRonAlone ()
        let passed, _ = stepped state (Action.None(seat 2))

        // 座位 1 第 2 巡摸进 1p 又摸切出去 —— 那是座位 2 的另一张和了牌，
        // 但座位 2 同巡振听，因此它那份里只有吃，没有 Ron。
        let discarded, events = stepped passed (Action.Dahai(seat 1, pai "1p", true))

        Assert.Equal<Event list>([ Dahai(seat 1, pai "1p", true) ], events)
        Assert.DoesNotContain(Action.Hora(seat 2, seat 1, pai "1p"), actionsOf (seat 2) discarded)
        Assert.Contains(Action.Chi(seat 2, seat 1, pai "1p", tilesOf "2p 3p"), actionsOf (seat 2) discarded)

        // 全部「过」之后这一局接着打，轮座位 2 摸牌。
        let passedAgain, drawn =
            stepped (othersPass (seat 2) discarded) (Action.None(seat 2))

        Assert.Equal<Event list>([ Tsumo(seat 2, pai "3z") ], drawn)
    ...
    let ``同巡振听解除之后，再打出的和了牌又能荣和`` () =
        ...
        Assert.Equal<Seat list>(seats [ 2 ], responding again)

        Assert.Equal<Action list>(
            [ Action.Hora(seat 2, seat 3, pai "1p"); Action.None(seat 2) ],
    ...
    let ``同巡两家都能荣和时，两家都进合法动作集`` () =
        ...
        Assert.Equal<Seat list>(seats [ 2; 3 ], responding state)
        ...
        Assert.Equal<Action list>(
            [ Action.Hora(seat 3, seat 0, pai "4p"); Action.None(seat 3) ],
            actionsOf (seat 3) state
        )
    ...
    let ``头跳开着时同巡双响只成立打牌者下家优先的那一家`` () =
        ...
        let first, events = stepped state (Action.Hora(seat 2, seat 0, pai "4p"))

        // 第一家宣言之后不立刻结束：还要等另一家答复，收齐了才裁决。
        Assert.Empty(events)
        Assert.False(GameState.isEnded first)
        Assert.Equal<Seat list>(seats [ 3 ], responding first)

        let ended, events = stepped first (Action.Hora(seat 3, seat 0, pai "4p"))

        Assert.Equal(1, List.length events)
        Assert.Equal<Seat list>(seats [ 2 ], horasOf ended |> List.map (fun hora -> hora.Actor))
    ...
    let ``头跳关掉时同巡双响都成立，按打牌者下家优先排序`` () =
        ...
        Assert.Equal<Seat list>(seats [ 2; 3 ], horasOf ended |> List.map (fun hora -> hora.Actor))
    ...
    let ``头跳关掉且三家和了不流局时（雀魂），同巡三响都成立`` () =
        ...
        let ended, _ = stepped second (Action.Hora(seat 3, seat 0, pai "4p"))

        Assert.Equal<Seat list>(seats [ 1; 2; 3 ], horasOf ended |> List.map (fun hora -> hora.Actor))
    ...
    let ``头跳开着时同巡三响只成立最靠前的一家`` () =
        ...
        Assert.Equal<Seat list>(seats [ 1 ], horasOf ended |> List.map (fun hora -> hora.Actor))

    [<Fact>]
    let ``头跳裁决的是实际宣言：优先的那家见逃时，靠后的那家成立`` () =
        let state = atTheDoubleRon ruleset
        let passed, _ = stepped state (Action.None(seat 2))
        let ended, _ = stepped passed (Action.Hora(seat 3, seat 0, pai "4p"))

        Assert.Equal<Seat list>(seats [ 3 ], horasOf ended |> List.map (fun hora -> hora.Actor))
        Assert.Equal<Furiten>({ Permanent = false; Doujun = true }, furitenOf (seat 2) ended)

    [<Fact>]
    let ``两家都见逃时这一局接着打，下家照常摸牌`` () =
        let state = atTheDoubleRon ruleset
        let passed, passEvents = stepped state (Action.None(seat 2))

        // 还有一家没答复，这一步只是收下一个答复，什么事实都没发生。
        Assert.Empty(passEvents)

        let continued, events = stepped passed (Action.None(seat 3))

        Assert.False(GameState.isEnded continued)
        Assert.Equal(1, List.length events)
        Assert.Equal(2, junmeOf (seat 1) continued)
        Assert.Equal<Seat list>(seats [ 1 ], responding continued)
    ...
    let ``答复过的座位不能再答复`` () =
        ...
        Assert.Equal<IllegalAction>(NotYourTurn(seat 2, seats [ 3 ]), rejected passed (Action.None(seat 2)))

        Assert.Equal<IllegalAction>(
            NotYourTurn(seat 2, seats [ 3 ]),
            rejected passed (Action.Hora(seat 2, seat 0, pai "4p"))
        )
    ...
    let ``响应阶段不接受打牌`` () =
        ...
        let hand =
            GameState.player (seat 2) state
            |> Option.map PlayerState.hand
            |> Option.defaultValue []

        Assert.Equal<IllegalAction>(
            NotYourTurn(seat 2, seats [ 2; 3 ]),
            rejected state (Action.Dahai(seat 2, List.head hand, false))
        )
    ...
    let ``摊好的两局各自以自摸和与荣和收尾`` () =
        ...
        Assert.Equal<Seat list>(seats [ 0 ], horasOf tsumo |> List.map (fun hora -> hora.Target))
        Assert.Equal<Seat list>(seats [ 0; 0 ], horasOf ron |> List.map (fun hora -> hora.Target))
        Assert.Equal<Seat list>(seats [ 0 ], horasOf tsumo |> List.map (fun hora -> hora.Actor))
        Assert.Equal<Seat list>(seats [ 2; 3 ], horasOf ron |> List.map (fun hora -> hora.Actor))
    ...
    let ``型成立但一个役都没有时，Ron 不出现在合法动作集里`` () =
        ...
        let discarded, events = stepped state (Action.Dahai(seat 0, pai "2m", true))

        // 座位 2 确实听牌、确实不振听，手牌加上 2m 也确实成和了型——
        // 挡住它的只有「无役」这一条。
        match GameState.player (seat 2) state with
        | Some player ->
        ...
        Assert.Empty(actionsOf (seat 2) discarded)
        Assert.Equal<Event list>([ Dahai(seat 0, pai "2m", true); Tsumo(seat 1, pai "9p") ], events)

        // 宣言也没用：引擎压根不在等它答复。
        Assert.Equal<IllegalAction>(
            NotYourTurn(seat 2, seats [ 1 ]),
            rejected discarded (Action.Hora(seat 2, seat 0, pai "2m"))
        )
    ...
    let ``同一手牌无役不能荣和，自摸却有门前清自摸和`` () =
        let state =
            ...
            |> driveUntil passive (fun state -> junmeOf (seat 2) state = 2)
        ...
        Assert.Contains(Action.Hora(seat 2, seat 2, pai "2m"), actionsOf (seat 2) state)

        let ended, _ = stepped state (Action.Hora(seat 2, seat 2, pai "2m"))
    ...
    let ``荣和时本场由放铳者一家付`` () =
        ...
        let ended, _ = stepped state (Action.Hora(seat 2, seat 0, pai "4p"))
    ...
    [<Fact>]
    let ``和了相关的拒绝理由有中文说明`` () =
        Assert.Equal("座位 1 声称自摸和 3索，但刚摸进的不是这张", IllegalAction.toDisplay (HoraTileMismatch(seat 1, seat 1, pai "3s")))

        Assert.Equal(
            "座位 1 声称荣和座位 2 打出的 3索，但刚打出的不是这张",
            IllegalAction.toDisplay (HoraTileMismatch(seat 1, seat 2, pai "3s"))
        )

        Assert.Equal("座位 1 的手牌加上 3索 不成和了型", IllegalAction.toDisplay (IllegalAction.NotAgari(seat 1, pai "3s")))

        Assert.Equal("座位 1 的手牌加上 3索 一个役都没有，不能和", IllegalAction.toDisplay (IllegalAction.NoYaku(seat 1, pai "3s")))

        Assert.Equal("现在没有可响应的牌，座位 1 无从响应起", IllegalAction.toDisplay (NothingToRespond(seat 1)))
