...
module PonChiTests =
    ...
    let private atTheRaceFor5s () =
        ...
        |> fun state -> fst (stepped state (Action.Dahai(0, pai "5s", true)))
    ...
    let private atTheDiscardOf3s () =
        ...
        |> fun state -> fst (stepped state (Action.Dahai(0, pai "3s", true)))
    ...
    let ``吃只吃上家：同样握着 4s 5s 的上家不被问`` () =
        ...
        Assert.Contains(pai "4s", handOf 3 state)
        Assert.Contains(pai "5s", handOf 3 state)
        Assert.DoesNotContain(3, waiting state)

        // 硬要吃：得到的是「现在不轮到你」——它压根不在被问之列。
        Assert.Equal<IllegalAction>(NotYourTurn(3, [ 1 ]), rejected state (Action.Chi(3, 0, pai "3s", tilesOf "4s 5s")))

    [<Fact>]
    let ``碰是任意他家：对家打出的牌照样能碰`` () =
        let state = atTheDiscardOf3s ()
        let ponned, _ = stepped state (Action.Pon(1, 0, pai "3s", tilesOf "3s 3s"))
        let discarded, _ = stepped ponned (Action.Dahai(1, pai "1z", false))

        // 座位 3 是座位 1 的对家（既不是下家也不是上家），它能碰。
        Assert.Equal<Seat list>([ 3 ], waiting discarded)

        Assert.Equal<Action list>([ Action.Pon(3, 1, pai "1z", tilesOf "1z 1z"); Action.None 3 ], actionsOf 3 discarded)
    ...
    let ``红 5 与正牌各算各的：亮哪两张是两条不同的合法动作`` () =
        ...
        let declared, pending = stepped state (Action.Pon(2, 0, pai "5s", tilesOf "5s 5sr"))

        Assert.Empty(pending)

        let ponned, events = stepped declared (Action.None 1)

        Assert.Equal<Event list>([ Pon(2, 0, pai "5s", tilesOf "5s 5sr") ], events)

        // 亮出去的是红 5，留在手里的是正牌 —— 红宝牌在副露里原样保留。
        Assert.Equal<Tile list>(tilesOf "5s 5s 5sr", nakiOf 2 ponned |> List.collect Naki.tiles)
        Assert.Contains(pai "5s", handOf 2 ponned)
        Assert.DoesNotContain(pai "5sr", handOf 2 ponned)
    ...
    let ``荣和压过碰：宣言的先后不影响裁决`` () =
        ...
        Assert.Equal<Seat list>([ 1; 2 ], waiting state)

        // 碰先宣言、荣和后宣言 —— 收齐答复才裁决，因此「先被问到」不等于「优先」。
        let ponFirst, events = stepped state (Action.Pon(2, 0, pai "5s", tilesOf "5s 5s"))

        Assert.Empty(events)

        let ended, _ = stepped ponFirst (Action.Hora(1, 0, pai "5s"))

        Assert.Equal<Seat list>([ 1 ], GameState.horas ended |> List.map (fun hora -> hora.Actor))
    ...
    let ``碰压过吃：吃的那家什么也没发生`` () =
        ...
        let chiFirst, _ = stepped state (Action.Chi(1, 0, pai "5s", tilesOf "6s 7s"))
        let ponned, events = stepped chiFirst (Action.Pon(2, 0, pai "5s", tilesOf "5s 5s"))

        // 只产出碰的那条事件，随后等碰的那家打牌。
        Assert.Equal<Event list>([ Pon(2, 0, pai "5s", tilesOf "5s 5s") ], events)
        Assert.Equal<Seat list>([ 2 ], waiting ponned)

        Assert.Equal<NakiKind list>([ NakiKind.Pon ], nakiOf 2 ponned |> List.map Naki.kind)
        Assert.Empty(nakiOf 1 ponned)

        // 吃没成，座位 1 的暗牌一张没动。
        Assert.Equal<Tile list>(handOf 1 state, handOf 1 ponned)
    ...
    let ``碰产出 mjai 事件、进公开副露，随后直接由碰的那家打牌`` () =
        let state = atTheDiscardOf3s ()
        let ponned, events = stepped state (Action.Pon(1, 0, pai "3s", tilesOf "3s 3s"))

        Assert.Equal<Event list>([ Pon(1, 0, pai "3s", tilesOf "3s 3s") ], events)

        // 副露是公开信息，字段与 mjai 的 pai / consumed / target 1:1。
        match nakiOf 1 ponned with
        | [ naki ] ->
        ...
        | AwaitingDahai phase ->
            Assert.Equal(1, phase.Actor)
            Assert.Equal<Tile option>(None, phase.Tsumo)
        | other -> failwith $"碰完应当等座位 1 打牌，实际是 {other}"

        Assert.Equal<Seat list>([ 1 ], waiting ponned)
        Assert.Equal<Tile option>(None, GameState.player 1 ponned |> Option.bind PlayerState.drawn)
    ...
    let ``鸣完那一手只有手切，牌数守恒：暗牌加副露的张数不变`` () =
        ...
        Assert.Equal(13, handOf 1 state |> List.length)
        Assert.Equal(11, handOf 1 ponned |> List.length)

        match GameState.player 1 ponned with
        | Some player ->
    ...
    let ``禁食替：碰完不能马上打回同一种牌，别的牌照打`` () =
        ...
        Assert.Contains(pai "3s", handOf 1 ponned)
        Assert.Equal<string list>([ "1m"; "2m"; "3m"; "9p"; "4s"; "5s"; "6s"; "1z" ], dahaiOf 1 ponned)

        Assert.Equal<IllegalAction>(Kuikae(1, pai "3s"), rejected ponned (Action.Dahai(1, pai "3s", false)))

    [<Fact>]
    let ``禁食替：吃完既不能打回被吃的那张，也不能打顺子另一端`` () =
        let state = atTheDiscardOf3s ()
        let chied, events = stepped state (Action.Chi(1, 0, pai "3s", tilesOf "4s 5s"))

        Assert.Equal<Event list>([ Chi(1, 0, pai "3s", tilesOf "4s 5s") ], events)
        Assert.Equal<NakiKind list>([ NakiKind.Chi ], nakiOf 1 chied |> List.map Naki.kind)

        // 吃 3s 用的是 4s 5s（两面搭子），因此 3s（现物）与 6s（筋）都不能打。
        Assert.Contains(pai "3s", handOf 1 chied)
        Assert.Contains(pai "6s", handOf 1 chied)
        Assert.Equal<string list>([ "1m"; "2m"; "3m"; "9p"; "1z" ], dahaiOf 1 chied)

        Assert.Equal<IllegalAction>(Kuikae(1, pai "3s"), rejected chied (Action.Dahai(1, pai "3s", false)))
        Assert.Equal<IllegalAction>(Kuikae(1, pai "6s"), rejected chied (Action.Dahai(1, pai "6s", false)))
    ...
    let ``连续鸣牌：中间被跳过的座位一张牌都没摸到，四家的 Junme 都不动`` () =
        ...
        let ponned, _ = stepped state (Action.Pon(1, 0, pai "3s", tilesOf "3s 3s"))
        let discarded, _ = stepped ponned (Action.Dahai(1, pai "1z", false))
        let again, events = stepped discarded (Action.Pon(3, 1, pai "1z", tilesOf "1z 1z"))

        Assert.Equal<Event list>([ Pon(3, 1, pai "1z", tilesOf "1z 1z") ], events)

        // 座位 2 在这两次鸣牌之间本该摸一张，被跳过了；鸣牌自己也不摸牌。
        Assert.Equal<int list>([ 2; 1; 1; 1 ], junmes again)
        Assert.Equal<Seat list>([ 3 ], waiting again)

        // 鸣完那家打完之后，摸牌的是**它的下家**，不是原本的顺序。
        let after, _ = stepped again (Action.Dahai(3, pai "9m", false))

        Assert.Equal<Seat list>([ 0 ], waiting after)
    ...
    let ``被鸣的那张仍在打牌者的河里，那家的河被记上「被鸣走过」`` () =
        ...
        match GameState.player 0 ponned with
        | Some player -> Assert.Contains(pai "3s", PlayerState.kawa player)
        | None -> failwith "座位 0 应当存在"

        let discarded, _ = stepped ponned (Action.Dahai(1, pai "1z", false))
        let again, _ = stepped discarded (Action.Pon(3, 1, pai "1z", tilesOf "1z 1z"))
    ...
    let ``鸣的不是刚打出的那张，或者那几张凑不成，都被拒`` () =
        ...
        Assert.Equal<IllegalAction>(
            NakiTileMismatch(1, 2, pai "3s"),
            rejected state (Action.Pon(1, 2, pai "3s", tilesOf "3s 3s"))
        )

        Assert.Equal<IllegalAction>(
            NakiTileMismatch(1, 0, pai "9p"),
            rejected state (Action.Chi(1, 0, pai "9p", tilesOf "4s 5s"))
        )

        // 手里确实有 4s 与 6s，但 4s 6s 配 3s 凑不成顺子。
        Assert.Equal<IllegalAction>(
            CannotNaki(1, NakiKind.Chi, pai "3s", tilesOf "4s 6s"),
            rejected state (Action.Chi(1, 0, pai "3s", tilesOf "4s 6s"))
        )

        // 1s 2s 3s 是顺子，但座位 1 手里没有 1s 2s。
        Assert.Equal<IllegalAction>(
            CannotNaki(1, NakiKind.Chi, pai "3s", tilesOf "1s 2s"),
            rejected state (Action.Chi(1, 0, pai "3s", tilesOf "1s 2s"))
        )
    ...
    let ``摸牌后阶段没有可响应的牌，鸣牌被拒`` () =
        ...
        Assert.Equal<IllegalAction>(
            ...
            rejected state (Action.Pon(context.Oya, 1, pai "3s", tilesOf "3s 3s"))
        )

    [<Fact>]
    let ``鸣牌相关的拒绝理由有中文说明`` () =
        Assert.Equal("座位 1 声称鸣座位 2 打出的 3索，但刚打出的不是这张", IllegalAction.toDisplay (NakiTileMismatch(1, 2, pai "3s")))

        Assert.Equal(
            "座位 1 用 4索5索 碰不了 3索",
            IllegalAction.toDisplay (CannotNaki(1, NakiKind.Pon, pai "3s", tilesOf "4s 5s"))
        )

        Assert.Equal("座位 2 振听，不能荣和 3索", IllegalAction.toDisplay (RonWhileFuriten(2, pai "3s")))

        Assert.Equal("座位 1 刚鸣完，这一手不能打 3索（禁食替）", IllegalAction.toDisplay (Kuikae(1, pai "3s")))
