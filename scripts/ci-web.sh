...
if [[ "$NO_BROWSER" == "1" ]]; then
  echo "== 浏览器里那七趟（曳光弹对拍 / 牌桌八项 / 黄金用例 / 牌谱导出两趟 / 两道 key 闸门）：按 JANPO_NO_BROWSER=1 跳过 =="
else
  # 七趟一条命令（票 56）：一个 Chrome + 一台 preview + 一台 dev server，七趟各开自己的
