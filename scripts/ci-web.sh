...
if [[ "$NO_BROWSER" == "1" ]]; then
  ...
  #
  # **这一趟里那份 6 MB 的产物不在场**（ADR-0006 边界 6：它不入版本控制，
  # 而这份脚本不要求任何人手里有它），因此这里跑到的是**降级那一路**
  # ——那也正是拉不动那 6 MB 的用户看到的那一路，而它有几条只在这一路上才跑得到的断言
  # （「算不动就整行不出现」）。
  #
  # **真推理那一档不在这份脚本里，在 `scripts/ci-baseline.sh`**（票 115）：
  # 它确认（必要时造）那份产物，再把这两条闸门的 `--asset` 那一档跑一遍；
  # CI 里它是 `ci.yml` 的 `baseline` job，与主 job 并行。两种形态都跑了，才是全的。
