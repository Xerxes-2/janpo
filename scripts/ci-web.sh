...
#
# 十六道：TS/JS 的格式与 lint（Biome）→ **TS 类型闸门**（tsc --noEmit）→ **Agent 层的用例**
# （node --test，回放录制的响应）→ **prompt 的语义不变量**（扫一批真实对局）→ Fable 编译
# → Vite 产物 → 之后是**浏览器里的那十趟**：**浏览器内跑引擎并与 dotnet 侧对拍**
# → **首页就是一局回放** → **牌桌上人看得见的八项** → **浏览器内跑黄金用例**
# → **浏览器内导出牌谱并回放** → **同一道闸门再走一整场** → **那道 key 闸门的反向自证**
# → **回显 key 的端点跑一手，牌谱里仍然没有它**
# → **URL 分享的载荷往返得回去** → **那道载荷闸门的反向自证**。
#
# **前六道各是一条命令，后十趟合成一条**（票 56）：它们从前各起一个 node 进程、一个 vite
# 服务器与一个 Chrome，现在跑在同一条跑道上（`web/scripts/verify-browser.mjs`：一个 Chrome
# 进程 + 一台 preview + 一台 dev server，十趟各开自己的 page/context）。**一趟没少、
# 一条断言没拆**——每趟调的就是 `verify-*.mjs` 里那个函数，单跑（`pnpm run verify:board` 等）
# 与合并跑跑的是同一段代码。十趟各自的来历写在 `verify-browser.mjs` 的那张表里。
#
# **那十趟各开哪个地址**（票 71）：只有第七道与第八道开 `/`（它俩量的就是首页），
# 其余八趟全开 `?table=1`——首页从此**自动播** Demo Paifu，而要点、要读牌桌的闸门
...
# 第八道是票 71 的验收：访客打开 `/` 什么都不用配，第一眼就是一桌牌在走
# （spec 的 story 1，ADR-0003 由 Demo Paifu 兑现）。四条断言：牌桌**在动**
# （隔一会儿采两次，手数必须不同）、页面上**没有配桌控件**、有一条**去 `?table=1`** 的路
# （真点过去，且那一页默认暂停）、页脚照旧。它顺带是「资产用 `fetch` 拉、不打进 bundle」
# 那条路径的唯一无头证据。
...
if [[ "$NO_BROWSER" == "1" ]]; then
  echo "== 浏览器里那十趟（曳光弹对拍 / 首页回放 / 牌桌八项 / 黄金用例 / 牌谱导出两趟 / 两道 key 闸门 / 分享载荷两趟）：按 JANPO_NO_BROWSER=1 跳过 =="
else
  # 十趟一条命令（票 56）：一个 Chrome + 一台 preview + 一台 dev server，十趟各开自己的
