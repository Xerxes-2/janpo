...
# → Vite 产物 → **浏览器内跑引擎并与 dotnet 侧对拍** → **牌桌上人看得见的八项**
...
# **牌桌八项**那一道是票 44 的验收：**渲染给模型**那一侧有黄金用例逐字段钉着，
...
if [[ "$NO_BROWSER" == "1" ]]; then
  echo "== 浏览器内的那几道（曳光弹对拍 / 牌桌八项 / 黄金用例 / 牌谱导出两趟 / 两道 key 闸门）：按 JANPO_NO_BROWSER=1 跳过 =="
else
  ...
  # 并且五个视角看到的位置必须逐字相同——位置的参照系是**副露方自己**，不是看牌桌的那个人。
  echo "== 牌桌上人看得见的八项 + 副露的位置就是来源（真对局）=="
  (cd web && node scripts/verify-board.mjs)
