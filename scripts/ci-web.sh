...
#
# 二十道：TS/JS 的格式与 lint（Biome）→ **TS 类型闸门**（tsc --noEmit）→ **Agent 层的用例**
# （node --test，回放录制的响应）→ **prompt 的语义不变量**（扫一批真实对局）→ Fable 编译
# → Vite 产物 → 之后是**浏览器里的那十四趟**：**浏览器内跑引擎并与 dotnet 侧对拍**
...
#
# **前六道各是一条命令，后十四趟合成一条**（票 56）：它们从前各起一个 node 进程、一个 vite
# 服务器与一个 Chrome，现在跑在同一条跑道上（`web/scripts/verify-browser.mjs`：一个 Chrome
# 进程 + 一台 preview + 一台 dev server，十四趟各开自己的 page/context）。**一趟没少、
# 一条断言没拆**——每趟调的就是 `verify-*.mjs` 里那个函数，单跑（`pnpm run verify:board` 等）
# 与合并跑跑的是同一段代码。十四趟各自的来历写在 `verify-browser.mjs` 的那张表里。
#
# **那十四趟各开哪个地址**（票 71）：只有第七道与第八道开 `/`（它俩量的就是首页），
# 第二十道两个地址都开（分享链接从 `?table=1` 复制、导入入口在 `/` 上），
# 其余十一趟全开 `?table=1`——首页从此**自动播** Demo Paifu，而要点、要读牌桌的闸门
...
if [[ "$NO_BROWSER" == "1" ]]; then
  echo "== 浏览器里那十四趟（曳光弹对拍 / 首页回放 / 牌桌八项 / 黄金用例 / 牌谱导出两趟 / 两道 key 闸门 / 分享载荷两趟 / 配桌三项开关 / 四 LLM 同桌 / 思考气泡 / 牌谱从外面进来的两条路）：按 JANPO_NO_BROWSER=1 跳过 =="
else
  # 十四趟一条命令（票 56）：一个 Chrome + 一台 preview + 一台 dev server，十四趟各开自己的
