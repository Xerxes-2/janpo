...
`./scripts/ci.sh` 跑的就是这个形态（它不要求任何人手里有这份产物），因此**它跑到的是降级那一路**。

**真推理那一档另有一条关卡**（票 115）：`./scripts/ci-baseline.sh`
——确认（必要时造）那份产物，再跑 `verify-review.mjs --asset` 与 `verify-baseline.mjs --asset`。
**本机与 CI 跑的是同一份**：CI 里它是 `ci.yml` 的 `baseline` job（与主 job 并行，产物由 Actions 缓存喂）。
两种形态**各有各的断言**，因此两边都要跑：不在场时才验得了「算不动就整行不出现」，
在场时才验得了强 AI 那一列的每一个数（普查实测：前者 4 条断言、后者 60 条，报告 115）。
...
1. **产物进 Actions 缓存**，键跟着「造它的那几份输入」走（两个脚本 + crate 源码与锁文件）。
   它们不变时部署只多花几秒（取一份 4.8 MB 的缓存，真 runner 上实测 2 s）；
   改了就重造一次（真 runner 上实测 **91 s**）。
   **CI 里 `baseline` 那个 job 取的是同一条缓存**（同一个键）：谁先造都算，不必各造一遍。
