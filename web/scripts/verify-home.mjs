...
const STAGE_VIEWPORTS = [
  ...
  // 1100×800 是**三栏摞成一列那一档**（票 125 补的）：从前这四个视口里
  // 1280 以上走三栏、390 那档由横向定尺寸，**没有一个落在窄档的桌面宽度上**——
  // 而那一档的纵向预留曾按三栏那套算，牌桌底边探出视口 7 px 而闸门全绿。
 ...
 *
 * 三条，都是意图级（判据 24：闸门只许断言用户看得见的保证，不许断言实现手段）：
