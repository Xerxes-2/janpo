...
//      点某一局的局号就落在那一局的开局帧；
//   ⑦ **这份牌谱不含推理**（票 76）：它是 bot 牌谱，因此**一个思考气泡都没有**，
//      而页面上得有一句话说清楚为什么没有——不说的话，「本来就不带推理」与「气泡坏了」
//      在页面上长得一模一样。带推理的那一版（票 79）会把这一条翻过来：那时该有气泡；
...
export async function verifyHome(lane) {
  ...
  try {
    ...
    // ⑦ 这份牌谱不含推理（票 76）：一个气泡都没有，而页面上说得出为什么。
    const bubbles = await page.locator('[data-testid$="-bubble"]').count();
    const said = await page.getByTestId("table-no-bubbles").count();
    if (bubbles !== 0) {
      missing.push(`首页那份牌谱一条决策记录都没有，却画出了 ${bubbles} 个思考气泡`);
    }
    if (said === 0) {
      missing.push(
        '首页上没有那句「为什么没有思考气泡」（[data-testid="table-no-bubbles"]）：' +
          "「本来就不带推理」与「气泡坏了」因此分不开（票 76）",
      );
    }
    ...
    console.log(
      "这份牌谱不含推理 ✓（一个思考气泡都没有，页面上说得出为什么；未结算不摆里宝牌、结算那一屏摆）",
    );
