...
//
// **它探得到那份产物在不在，两种形态各验各的**（ADR-0006 边界 6：那 6 MB 不入版本控制）：
// 不在场时 ⑩ 的阳性对照量的是「请求真的发出去了」（回的是 404），第三程量的是「算不动就整行不出现」；
// 在场时 ⑪⑫⑬⑭⑮ 才量得到（普查实测：在场多跑 52 条断言、不在场多跑 4 条，报告 115）。
// **两种形态 CI 里都跑**：`scripts/ci.sh` 跑不在场那一路，
// `scripts/ci-baseline.sh`（`ci.yml` 的 `baseline` job，产物由 Actions 缓存喂）跑在场那一路。
// 本机想跑在场那一路：按 `web/public/baseline/README.md` 造一份放进去就行（它自己探得到）。
//
// 选项：--budget ms（页面内驱动的时限）、--peek N（走多少手之后做①那一条）、
// --sample-every N（第⑮条每隔几手抽一手去 node 那侧重问）、
// --asset（本机演习：那份产物不在就当场报错，而不是静静地走降级那一路）。
//
// **强 AI 那一行上那几个数（概率 / 几条 / 排第几）不在这里溯源**：产物不在场时那一行根本画不出来，
// 而这份闸门两种形态都要跑，写在这里就是一条在一半的形态里永远执行不到的断言（判据 3）。
