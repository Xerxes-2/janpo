...
//
// **视角一律用那一页自己的默认值**：票 82 之后两页都是**上帝视角**
// （票 81 把视角变成了气泡的闸门，坐座位 0 的话把模型摆在别席的主持人第一眼看不到自家模型说话；
// 而 `humanSeated` 恒 false，没有可泄露的对象）。**两张图都不在这里改视角**：
// 图应当是页面本来的样子——想看「他家暗牌看不见」那一份投影，页面上按一下「座位 N」。
//
