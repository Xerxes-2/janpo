...
/// 哪几次荣和，是复盘（票 90）第一件要问的事；而票 89 的时限**到点自动过**时，
/// 同一条记录还要能说出「这一次不是他按的」——那一格由那一票加，本票不预留空字段。
type HumanPass = {
    /// 第几手（`Table.Turns`，跨局累计）。
    Turn: int
    /// 哪一席（本地真人那一席）。
    Seat: Seat
    /// 这一次「过」放掉的那几条的中文 label，按合法动作集的既定顺序；**「过」本身不在里面**。
    Skipped: string list
}
...
module HumanPass =
    ...
    let toDisplay (pass: HumanPass) : string =
        ...
        | [] -> $"第 {pass.Turn} 手你按了「过」"
        | skipped -> $"第 {pass.Turn} 手你按了「过」，放掉了：" + String.concat "、" skipped
