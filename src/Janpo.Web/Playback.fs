...
module Speed =
    ...
    ///
    /// **改这几个数要两处一起改**（票 112）：`web/scripts/verify-playback.mjs` 里那张 `DESIGN`
    /// 表是它们的**期望值**，浏览器那一趟拿实测的「手/秒」与它对。那份期望**故意不从这里
    /// import**——同一个实现当自己的期望就是恒真式，改坏这几个数时闸门会跟着改口。
    /// 本机实测（报告 112）：四档各 1.65 / 3.31 / 6.61 / 16.5 手每秒，是设计值的 0.986–0.994。
