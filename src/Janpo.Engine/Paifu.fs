...
/// ——人格与模板**一局内不变、局间换得动**（CONTEXT.md 的 `Persona`；执行者是
/// `Janpo.Web` 的 `TablePage.Rendering`），主持人在局间换了一版就多一条，
/// 而每条决策记录靠自己的 `RenderVersion` 指得回当时那一份。这就是它是一个**列表**的理由。
type Preamble =
    {
        /// 用这一份 preamble 的座位。
        Seat: Seat
        /// 渲染版本号（`模板 id@模板哈希.渲染器摘要`）。**算出来的，不是手填的**：
        /// 手填的数字没有任何东西保证有人改措辞、改排版时会 +1（裁决 29b-A、票 43）。
...
type DecisionRecord =
    {
        ...
        PromptTail: string
        /// 渲染版本号。**它是这条记录与 `Prompting.Preambles` 之间的键**，
        /// 也让 M2 看命中率掉时一眼归因到「换了渲染」。
        ///
        /// **v3 起是 `模板 id@模板哈希.渲染器摘要`（票 43）**：前一截是措辞（人格、规则说明、
        /// 抬头与五张措辞表），后一截是**排版的那几行代码**（Agent 层的
        /// `prompt.ts` / `history.ts` / `wording.ts` / `template.ts`）。v2 只有前一截，
        /// 那一版的值**说不了渲染器换没换**，`Paifu.Version` 分得清是哪一种。
        /// v1 牌谱里没有这一项，读出来是空串。
        RenderVersion: string
...
module Paifu =
    ...
    ///
    /// **2 → 3 是票 43 涨的**：`render_version` 的含义从「模板内容的哈希」变成
    /// 「模板内容的哈希 **+ 渲染器代码的摘要**」。字段名与结构一字未改，变的是那一串
    /// **说得了什么**：v2 的值只能回答「模板换没换」，v3 的值连「排版的代码换没换」一并回答。
    /// 不涨的话，拿两份牌谱对比 `render_version` 的人无从知道自己手里那个值是哪一种承诺。
    let version = 3

    /// 这个版本的引擎读得动的格式版本。加一个版本就在这里加一项，并让解码器认得它。
    let supported: int list = [ 1; 2; 3 ]
    ...
    ///
    /// **v2 与 v3 写出来的形状逐字相同**（票 43 改的是 `render_version` 那一串的含义，
    /// 不是字段形状），因此这里只分 v1 与其余两支；区分 v2 / v3 的是牌谱头那个 `version`。
