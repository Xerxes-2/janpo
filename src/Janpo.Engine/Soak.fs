...
module Soak =
    ...
    /// 一家整局只打幺九牌且一张不被鸣），随机探索走不到。
    /// `Soak.uncovered` 会把这一批里没走到的如实列出来，因此「没覆盖」看得见。
    ///
    /// **这三种在跑批之外现在各自有了什么（票 57 扫了 12,188 场真牌谱）**：
    /// 四风连打 82 局、流し満貫 17 局，两者已进真牌谱对拍的 CI 固件（各 4 / 3 局，
    /// `PaifuDifferentialTests` 里有条断言数着次数）；**三家和了仍旧没有任何自动闸门**——
    /// 牌谱里有 6 局（天凤 JSON 确认），但上游把那三条荣和宣言删成了裸 `ryukyoku`，
    /// 重放喂不出来；它的覆盖仍只有 12 票摊好的剧本用例。
