...
module Ukeire =
    ...
    let calculate (kindSet: TileKindSet) (visible: Tile list) (hand: HandShape) : Result<Ukeire, UkeireError> =
        if not (HandShape.isAwaitingDraw hand) then
            ...
            // 可见张数 = 手牌自己的 + 传入的；`HandShape.counts` 是内部数组，必须先复制再改。
            let seen = Array.copy (HandShape.counts hand)
            ...
            | None ->
                ...
                let improving =
                    [
                        for tile in TileKindSet.kinds kindSet do
                            // 手里已有 4 张的牌种摸不到第 5 张，不是有效牌。
                            if HandShape.countOf tile hand < 4 then
                                match HandShape.add tile hand with
                                | Ok drawn when Shanten.value (Shanten.calculate kindSet drawn) < Shanten.value current ->
                                    yield tile, 4 - seen.[Tile.kindIndex tile]
                                | _ -> ()
                    ]
