...
**Status:** done（报告：run/reports/133-final-scorecard.md）
...
- [x] 终局那一屏（`table-result` 附近）多一张表，一席一行，列：席位·风 / 选手·档 / 顺位·终点 / 和·铳 / 兜底 / 重试 / 输入·输出 tok
- [x] 那几个聚合是**引擎侧的纯函数**（吃一份 `Paifu`，出一条 per-seat 记录），dotnet 侧有用例钉住
- [x] 一枚「复制记分卡」：把同一张表出成一段纯文本进剪贴板（贴 issue / 贴群那种）
- [x] **纯文本里不许出现 key**：照抄票 34 那道检查的形状，闸门里要有一条「记分卡文本里搜不到 key」
- [x] 无头闸门：终局之后表在、四行齐、每一格的 `data-*` 与引擎直接算的那份对拍（同 `verify-review.mjs` 的形状）
- [x] 还没终局时**整块不在 DOM 里**（同 `ReviewShown.Hidden` 那条规矩）

## 边界

- [x] 不动牌谱格式（`Paifu.Version` 一个字不涨）、不动 ADR-0002
- [x] 不碰右轨那四行席位卡的形状（票 124/126 的地盘）
- [x] 不做 PNG——那是票 134
- [x] 不动 `Playback` 世代号、不加 localStorage 键
