...
- [ ] Agent 层是 TS：输入决策包 JSON，输出一个动作 id 的 Promise；**它不认识 `Action`，也拿不到 `GameState`**
- [ ] `choose_action` 工具的参数 schema 由决策包的动作 id 动态生成（`StringEnum` 约束 + 一句话理由字段）
- [ ] 座位级配置：provider / 模型 / API key / 思考预算 / 超时；key 只进 localStorage，绝不外发到本平台
- [ ] 兜底闭环：解析失败或 id 非法 → 重试上限 2 次 → 仍不行则**摸切**（Bare 档的兜底策略）；
      超时与 provider 报错走同一条兜底路径
- [ ] 牌桌上能看出某一手是兜底出来的（不是静默替换）
- [ ] Agent 层确定性测试：用**录制的响应**覆盖四类路径 —— 合法输出、非法牌 / 越界 id、格式跑偏、超时；
      **CI 里不调真实 API**
- [ ] 浏览器里 LLM 坐一席 + 三随机 Player 打完一个 Kyoku
- [ ] **断电演习**：故意配一把坏 key，整局照样打完（全程兜底），页面上有明确的错误状态提示
