...
**做完了**：报告 `run/reports/101-pages-baseline-asset.md`。选的是**路 3 的一个变体**
——Actions 缓存**那 6 MB 产物本身**（不是 cargo 的 target 目录），没命中就现造，造不出来不阻断部署。
稳态部署 92 s → 约 100 s；重造那一次估 +1.5–2.5 分钟。**「线上拿得到 200」这一条只能上线验**
（报告 ⑧ 给了判据与回滚）。
...
- [x] 三条路各给一组实测数（哪条没量到就写明为什么），**选一条落地**，理由写进报告
      （报告 ①；路 2 的「上传耗时」没量到——本票无远端权限，造不出 release）
- [x] **部署时长的代价要写在明面上**：今天 pages.yml 跑多久、改完跑多久
      （报告 ②：改前 **92 s**（GitHub API 读的 run #127 真数，逐步拆开）；
      改后命中约 **100 s**、未命中估 **3–4 分钟**）

## 要什么行为

- [ ] 线上 `https://<owner>.github.io/janpo/baseline/janpo-baseline.wasm` **拿得到 200**
      ——**只能上线验**（本票无远端权限）。流水线已就位且在本机用 `act` 跑过四趟（报告 ⑤）；
      推 main 之后怎么核与怎么回滚写在报告 ⑧
- [x] **许可随产物上线**（ADR-0006 边界 4，`web/public/third-party/` 里那两份已经在发布了）：
      确认 `LICENSE-akagi.txt` / `NOTICE-akagi` 在 `web/dist` 里、页脚那条声明指得到它们
      （报告 ③：三份都在，并做成了一道**红得起来**的闸门 `scripts/check-pages-dist.sh`）
- [x] **懒加载不许被破坏**（边界 1）：产物在站点上之后，
      首页与不选那一档的对局**照旧一个字节都不拉**——`verify-baseline.mjs` 那条断言照旧绿
      （报告 ④：产物在场那一档第 ① 趟原样绿）
- [x] **降级那一路不许失效**：产物存在时它测不到，所以那一趟要保留一个「资产地址改坏」的构造
      （今天 CI 里它是主路，改完之后别让它变成一条没人跑的路——判据 3）
      （报告 ⑥：CI 里那 6 MB 仍不在场（本票一个字没动 `ci.yml`），降级照旧是主路；
      那个构造走的是 `route(...).abort`，**与产物在不在场无关**）
- [x] 部署产物的**体积与缓存头**：6 MB 每次都重下还是 immutable 缓存，写清楚
      （报告 ⑦：Pages 一律 `max-age=600` + ETag，**拿不到 immutable 且改不了**；
      但“每次重下”不成立：懒加载 + 304）

## 闸门

- [x] 本地跑通那条构造链（`probe/akagi-wasm/fetch-upstream.sh` + cargo），
      **cargo 缓存在 `/tmp/janpo-probe-target`（约 900 MB，完好），用 `CARGO_TARGET_DIR` 指过去，
      不许 `cargo clean`**
      （那 916 MB **一次都没动过**；冷跑用的是 `/tmp/janpo101/target-*` 两个新目录）
- [x] `./scripts/ci.sh` 全绿；**`verify-baseline.mjs --asset` 那一档在本机跑过一次**（真推理）
      （报告 ④：`ci.sh` 全绿跑了两遍，**一遍产物在场、一遍不在场**）
- [x] workflow 改动要能自证：**不许「推上去看看」**——用 `act` 或把关键步骤在本机复现，
      跑不了就在报告里写清「这一步只能上线验」并给出回滚办法
      （报告 ⑤：`act` 四趟，两趟是破坏实验；只能上线验的四件事 + 三级回滚写在 ⑧）
