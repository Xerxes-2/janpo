...
## 票 133（终局记分卡）—— 实现 agent 自裁的几件

**133-1｜作废掉的那几次问话不进记分卡的 tok 列，差额当场说出来。**
记分卡每一格都是**牌谱**的聚合，而 `VoidedAsk`（花了钱、没落子）不在牌谱里（裁决 110）。
于是四行相加 ≤ 牌桌那条账单行（`Table.usage` 含它们）。被否决的两条：把 voided 摊进某一席
（它没落成一手，摊给谁都是编的）、干脆让账单行也不含它们（那会让页面上的总额小于人真花的钱）。
处置：加一句 `table-scorecard-voids`，只在真有付过费的作废时才画，把「另有 N 次问话花了钱、
没落子（X 输入 / Y 输出 tok）」写出来——票 39 那条「同一屏上不许两个数并排站着不解释」。

**133-2｜「放铳」那一格的标识符叫 `HoraTargeted`，不叫 `Houjuu`。**
`CONTEXT.md` 里没有「放铳」的罗马字词条（只在散文里出现过两次），而改术语表要单票授权，
我没有。`HoraTargeted` 复用的是引擎里已有的两个词（`Hora` + `Hora.Target`），一个新词都没造。
**提案留给主人**：如果值得，给 `CONTEXT.md` 加一条 **Houjuu（放铳）**——
「`Hora` 事件里 `Target` 指着这一席而 `Actor` 是别人；自摸时 mjai 把 `Target` 写成和了者自己，
那不算放铳」——然后把这个字段改名。中文那一列今天已经写着「和 · 铳」（渲染是单向出口）。

**133-3｜「席位 · 风」取的是最后一局的自风，这一列对一整场有歧义。**
票面指定来源是 `Board.ofTable`，同屏的座位卡读的也是它（不许第二份判据）。
后果：首页那份 Demo 打到南 4 局，记分卡四行因此写着「南 / 西 / 北 / 东」，
而读者多半以为那是起家风（东 / 南 / 西 / 北）。三条路各有代价：
① 照旧（同屏一致，但抬头会被误读）；② 改成起家风（整场恒定、可比，但与座位卡不一致）；
③ 那一列只写座位号、不写风（信息少一格）。**我选①**（票面指定的来源，改了就是第二份判据），
**待主人裁**是不是改抬头措辞或换成②。

**133-4｜记分卡多带一个 `Asked`（问过几次），但不占屏幕上的列。**
兜底与重试没有分母读不了（bot 席恒 0，而 0 与「一手都没兜底」不是一回事），
而票 135 的首页小表要的正是**兜底率**。它只上 `data-asked` 与 `SeatTally.Asked`，
屏幕上那张表仍旧是票面点名的七列。

**133-5｜逐格对拍差点是一条恒真式，靠第三锚点补的。**
`ScorecardCheck.tally`（闸门那侧）与页面那侧在七列上共用引擎里同一段 `Scorecard.tally`
——那一段本来就该只有一份（判据 11），不该为对拍再写第二份。实测：把 `Attempts - 1`
改成 `Attempts`，「逐格对拍 36 格」**全绿**。处置是在 `verify-scorecard.mjs` 里加
`tallyFromPaifu`：node 侧照**规则**把那份 JSON 重数一遍（判据 6 的「第三个锚点」）。
加完之后同一次破坏当场红出四条。**顺位与终点不进第三锚点**：那两列两侧本来就是两条路。

**133-6｜（已了结）票面那句「牌谱里根本没有模型身份」不成立 → 调度器松了边界 → 已落地。**
第一版我照票面把整格写成「牌谱没记」并报了上去。**调度器核过并认账**
（`Event.fs:116` 的 `StartGame of names: string list`，回放名牌画的就是它），当场松了边界，
我照新边界重做了这一列（详见 133-10；全文在 `reports/133-final-scorecard.md` §八）。
**这一条不再是待审项。**

**133-7｜票外发现，只描述不编号（判据 17）：`verify-export.mjs --to-end` 单跑时页面冒
5 条 React `Maximum update depth exceeded`。** 合并跑道上不冒（那一趟只跑 0.8 s），CI 因此是绿的。
我把票 133 改过的全部文件回退到 `46d18444`、把三个新文件挪走、重新 fable，**照样必现**
⇒ 与本票无关。全仓库唯一的 `useEffect` 在 `ReviewPanel.useReview`（终局那一刻 `setConsulted`），
是唯一嫌疑，我没继续查。它是判据 22 那一族：**只在冷启动 / 慢机上开口的假绿**。

**133-8｜code-review 逮到的四条 blocking 当场修完（Standards 轴）。**
① 闸门拿「牌谱没记」这句**中文**当断言 —— 判据 24 的自测口诀直接命中（改措辞、视觉不变，它会红）。
处置：立 `ScorecardPlayer`（`Nameplate` / `Unrecorded` 两个 case，判据 12）+ `toWire`，
DOM 上多一个 `data-player-source`；闸门读 wire，中文只印给人看。
② wire 与 DOM 上造了 `houjuu` 这个 `CONTEXT.md` 没有的词（F# 侧本来避开了，转手在 JSON 上造了回来）
—— 一律改成 `hora_targeted` / `data-hora-targeted`，133-2 那条提案照旧挂着。
③ 「复制记分卡」那两条消息一条 dotnet 用例都没有，而注释里写着三条不变量（判据 2），
`failed` 那一支一次都执行不到（判据 3）—— 补 3 条用例（含**把 Live 那一桌真打到终局**才点得着那一枚）。
④ **133-1 那条自裁的执行体在真语料上执行 0 次**（判据 3：为 0 的当场喊停）：回放 `paidVoids` 恒空。
处置：把差额与那句话搬进 `ScorecardView.voidedGap` / `voidedSaid`（纯函数），
补一条造了两笔 `VoidedAsk` 的用例 + 一条阴性对照。

**133-9｜文本对拍原来是 `includes`，逮不住列错位；改成逐列切开比之后仍不够。**
Spec 轴的 nitpick 指出「0」那种格在任何一行里都命中得了。改成按 `|` 切、逐格相等之后，
随手试「把兜底与重试两列对调」——**还是不红**：屏幕与文本读的是同一份 `cells`，一起换位置。
于是补了一段「**看得见的那一格与同一行的 `data-*` 对得上**」（28 格），同一次对调才红出来。
这是判据 6 在渲染层的同一形状：**两侧同源的对拍是恒真式**，右侧必须另找一个锚点。
残留一条**没有执行者**、写在这里备案：表头那七个中文标签与 `cells` 的顺序若一起被改错
（headers 与 cells 同在 `ScorecardView`），今天没有任何断言开口——
要它开口就得断言中文标签，而那正是 133-8 ① 刚拆掉的东西。**我选择不加**，把它记在这里。

**133-10｜「选手 · 档」按调度器松边界后的结论重做：身份取牌谱、档位取名牌。**
`ScorecardPlayer` 从两态改成 `Named of name * ScorecardTier` + `Unknown`，
`ScorecardTier` 三态（`Set` / `NotApplicable` / `Unrecorded`）——**判据 12**：
「这一席没有档位」（bot / 强 AI 基线不走 prompt）与「这份牌谱没记档位」（回放）
在页面上是两句不同的话，压成一个之后 bot 席会被写上「档位牌谱没记」，那是句假话。
`data-player-source` 因此有四个值（`tiered` / `no-tier` / `tier-unrecorded` / `unrecorded`），
外加 `data-player-name` / `data-player-tier` 两半各一格。

来源各只有一处：**身份** = `start_game.names`（回放取 `ReplayTable.Ready` 那一份，
与名牌同一份；Live 取 `Roster.names`）；**绝不解析 `DecisionRecord.Output`**（provider 的原始回执，
F# 不解释它）。**档位** = 新加的 `SeatingPlan.tiers`——为它把 `nameplates` 拆成
`plateWho` + `plateTier`，两个出口共用同一个 `plateTier`，「这一席写不写档位」这条判据
因此物理上只有一处（既有 `SeatingPlanTests` 16 条一条没红，行为逐字未变）。

**Live 那一桌两处仍旧不同，是故意的**：名牌写本机私人档案名，记分卡写 `provider/model`
——记分卡是要被带走的东西，私人叫法带出去谁也认不出是哪个模型（`ModelProfile.Name` 那条术语）。
**档位那半句两处逐字相同。**

闸门三处，各先红过一次：回放屏「身份格与名牌逐字相同 + 档位是『档位牌谱没记』+ wire 是
`tier-unrecorded`」（拿旧实现跑新断言，红 12 条）；Live 侧「身份 = 导出牌谱那一列 `names`」
（改用私人档案名 → 红 4 条）与「档位 = 名牌那半句」（给 bot 席也写一个档位 → 红 8 条）。
dotnet 侧新增 6 条（含「`names` 是空串才整格退回『牌谱没记』」带阳性对照）。

顺带：那一格变长后最后一列被挤出视野，**只让「选手 · 档」那一列折行**（数那几列不折，
折了上下四家就对不齐），截图核过。
...
**M6-12｜票 133 集成（调度器，2026-08-30）。** `ci.sh` 全绿。两处冲突都是**纯追加**
（`DECISIONS.md` 与 `styles.css` 各一处：133 的记分卡段与 138 的一行式开桌段都在文件末尾新起），
按 `resolve-append-conflicts.py` 解，**没让它猜**。

**我自己跑了那趟闸门（`node scripts/verify-scorecard.mjs`），不只读报告**：
逐格核过 36 格 + 复制文本里 28 格 + 「491 字符里搜不到那把 key」。
身份那一列四行与右轨名牌**逐字相同**——133-6 那个「同一屏两个说法」的毛病确实没了。

**闸门印出来的那张表暴露了 133-3 的严重程度，比报告里说的更值得裁：**
...
**「席位 · 风」取的是末局自风。** 东风战里座位 0 是起家、开局是东家，而整场打完之后
这张表写着「座位 0 南」「座位 3 东」。**风每一局都在转，而记分卡是整场的结论**——
写一个瞬时的风是误导，且它正好摆在最左边那一列、是人读这张表的第一眼。

**我的判断：那一列该只写席位（座位 0…3，或「起家」），风整个去掉。**
这张表的四行是**四个选手**，不是四个方位；要方位的人看牌桌，不看记分卡。
**不裁的后果**：记分卡这个「可以贴出去的结论」第一列就是错的信息，而它被贴出去之后没人能纠正。
⇒ 等主人一句话，我开一张小票改（连带闸门那条逐格对拍要跟着改）。

**仍挂着的另一条 133-2**：`Houjuu`（放铳）要不要收进 `CONTEXT.md`。
术语表改动要单票授权（AGENTS.md 硬约束 5），等主人点头后并进上面那张小票。

**W1 收官**：139 ✅ 138 ✅ 143 ✅ 133 ✅，四票全绿落地，集成头 `037a34ad`。
本批到此逮到的「量点没落在被关心的那件事上」共三条（139 量合数、143 量 loading 帧、144 走不到终局），
其中 143 与 144 那两条**都不是这一波写坏的，是本来就在那里**——这本身值得记一笔：
**闸门的覆盖缺口只有在有人往它旁边加东西时才会暴露。**

**M6-13｜主人裁：「144 排 W3，先把 133 和 143 收了」（2026-08-30）。** 我按两件事执行：

**① 票 133 的两条挂账 ⇒ 开票 145**（第一列去掉风、`Houjuu` 收进 `CONTEXT.md`）。
**术语表那一次授权我按「收了」读作已给**（AGENTS.md 硬约束 5 要单票授权，而这两条正是我上一条
明着问的两件事）——**票面里把授权范围写死到只这一个词条**，顺手看见别的词写进 `DECISIONS.md` 等下次。
若主人本意不含术语授权，说一声，145 只做第一件。

**② 票 143 那条 58 字挂账 ⇒ 不开新票，转成票 137 票面上的硬要求。**
「收了」不等于「现在砍文案」：那 58 字今天有用（它是「打完之后去哪看」的指路），
而票 137 正要给首页加「它为什么打这张？」——**两句话是同一件事的两半**。
∴ 我把它写进 137 的票面：两句合成一句、合完把首页上限从 125 收下来、**不许靠抬上限过关**。
这样它**不再是一条挂着的待裁**，而是一条有执行者、有验收的要求（判据：待裁项要么被裁，要么变成票面上的一行）。

**③ 票 144 确认排 W3**（主人裁的），理由与我原来的排法一致：它落在 `ReviewPanel.useReview` 上，
而票 136 要往同一处加「对局中的右轨」——两票撞在同一个文件。

**W2 我没有自作主张派出去**：主人这一句只回了 144 的位置与 133/143 的收尾，
没说 136 / 141 开跑。派工只在得到明确指令之后（排班表的「派工中」只在派工调用成功返回之后写，同一个规矩）。

**票 145｜记分卡第一列去掉风、`Houjuu` 收进术语表（报告：`run/reports/145-scorecard-loose-ends.md`）。**

  - **145-1：最左那一列只写「座位 N」，风整个去掉；表头「席位 · 风」→「席位」。**
    被否决的两条：① 改成起家风（整场恒定、可比）——那要读者在脑子里再套一层「哪个风对应哪一席」，
    而**座位号页面上到处都是**（名牌、视角按钮、危险度面板、`data-anchor`），多一套说法就是多一套要学的东西；
    ② 改成「起家 / 二家 / 三家 / 四家」——同上，票面明着否掉了。
    理由是**这张表是整场的结论，而风每一局都在转**：133 那一版取末局自风，东风战打完之后
    最左那一格写着「座位 0 南」，可座位 0 正是起家、开局的东家。它摆在第一列、是人读这张表的第一眼，
    而这张表是要被贴出去的——贴出去之后没人能纠正。
    **这不是「改期望值迎合实现」**：改的是「页面该显示什么」这个决定（票面裁的），
    实现与断言是跟着那个决定一起动的，且断言只往更硬改（见 145-3）。
  - **145-2（越界，我没动，交调度器）：`HoraTargeted` 该不该改名叫 `Houjuu`。**
    133-2 当初起这个名字的**理由原话是「`CONTEXT.md` 里没有「放铳」的罗马字词条」**——
    票 145 收了词条，**那条理由今天就过期了**（判据 15：前提被后续票拆掉时回头重问那条决策）。
    而这一票的授权只到词条本身，∴ 我只把两处已经变成假话的注释改成指向 145-2，代码一个标识符没动。
    改名的代价我估了（没写）：`SeatTally.HoraTargeted` 一个字段 + wire `hora_targeted`
    （`ScorecardCheck` / `Scorecard` 的 JSON）+ DOM 钩子 `data-hora-targeted` + `verify-scorecard.mjs`
    的 `COLUMNS` / `ANCHORED` / `tallyFromPaifu` 各一处 + 两个测试文件——**一趟纯改名，无行为**。
  - **145-3：新断言四条，先红一次，红的原话进报告。** dotnet 两条（屏幕上那张表 / 复制出去那段文本，
    最左一列逐格核「一个风字都没有」，并**带一条非退化守卫**（判据 3，不是判据 21 的阳性对照——它修前修后都绿）：
    先证明这一场末局自风确实已经转过了，否则那两条只是在一张「风恰好没转」的表上成立。
    **判据 21 那一对在浏览器闸门里**：③「第 0 格逐字是座位 N」（阳性）与 ③c「没有风字」（阴性）
    共用同一次拖动、同一批节点）；浏览器两条（③c 表头 + 四行、⑥b 文本那一列）。
    另把 ③ 里席位那一格的**前缀比法改成逐字相等**——前缀比法会放过任何跟在后面的东西，
    而 133 跟在后面的正是那个风。把实现回退之后 dotnet 红 2 条、浏览器红 14 条（EXIT=1）。
  - **145-4：风字表里连「风」这个字自己一起收。** 头一版只查「东南西北」，
    结果**表头「席位 · 风」一声不响地绿着过去了**——只改格子不改表头，那道闸门逮不住。
    判据 1 的又一例：新口径的漏面就在「同一件事的两半」那条缝上。
  - **145-4b：③c 的「扇到 0 个就红」兜底头一版是死代码，评审逮到了（判据 12 / 22）。**
    表头与四行拼成一个数组一起数，而表头那一格恒有一格 ⇒ `windless === 0` 永远走不到，
    而报告同一页上写着「各带一条兜底」——**那是一张空收据**。改成表头单独核、计数只数行，
    `shown.rows` 空掉时它才真是 0。⑥b 那条顺带改准：格数对不上时报真因，不再只说「在空转」。
  - **145-5：`Houjuu` 词条写清执行者与边界，逐条去核过。** 执行者是 `Scorecard.horaOf`
    （`Scorecard.tally` 里那一段）→ `SeatTally.HoraTargeted`。**「自摸不算」不是一句注释，是匹配次序**：
    `Actor = seat` 那一支排在前面，自摸（`Actor = Target = seat`）因此走不到 `Target = seat` 那一支
    ——我把这句写进了词条，因为「减掉一笔」那种说法会让后人以为代码里有一道减法，而没有。
    守着它的两条用例（`ScorecardTests` 的「自摸只记和了」「双响时…记两笔放铳」）与
    `verify-scorecard.mjs` 的第三锚点，我逐条打开核过它们今天还在、还跑得到。
    边界另写了一条：**双响时放铳那家记两笔**（两条和了事件各指着它一次）。
  - **越界发现，记账不动**：**「起家是座位 0」这件事今天在页面上看不出来。**
    上帝视角的参照系确实是起家（`Board.anchor`），但**票 121 把「以起家（座位 0）为下方」那句话删了**
    （主人裁的），今天只剩 `data-anchor` 这个给机器看的钩子。名牌上写的是「座位 N・<自风>」，
    而自风每局在转，读不出「谁是起家」。**票面说「看不出也别在这一票补」，∴ 我一个字没加**，交调度器裁。
    我的判断：记分卡去掉风之后这件事**没有变得更糟**（原来那一列写的是末局自风，本来就答不出起家），
    但它是一条真缺口——要补的话该补在牌桌那一侧（名牌或参照系那一句），不该补进记分卡。

**M6-14｜票 145 集成（调度器，2026-08-30）。** `ci.sh` 全绿，无冲突。
干净重跑 `verify-scorecard.mjs`：第一列现在是 `席位 / 座位 0…3`，表头与四行、
以及复制出去那段文本里**一个风字都没有**；其余 28 格逐格照旧对得上，479 字符里搜不到 key。

**我自己制造了一次假红，记下来。** 我在 `ci.sh` 正跑的**同一个工作副本**里手工跑了那趟闸门，
它读到的是半路重建的 Fable 产物 ⇒ 报出「最左一列还有风字」。**那个红长得和真缺陷一模一样。**
`dispatch.md` 那条「假红先怀疑基础设施」这次应验在调度器自己身上
（原案例是四个工作区抢同一个写死端口）。
**以后的规矩**：集成时要手工核闸门，**等 `ci.sh` 跑完再核，或换一个工作区跑**——
两个进程抢同一份 `web/src/generated/` 就是抢同一个端口的同一种事故。

**agent 自己 code-review 逮到一条，值得记**：它 ③c 那条「扇到 0 个就红」的兜底**是死代码**，
而报告里声称它存在。**这正是本批第四次「断言在，而它守的不是那件事」**——
前三次是 139 量合数、143 量 loading 帧、144 走不到终局。
这一次是 agent 自己在交付前逮到并修了的，**做法对**。

**两条交我裁的**：

  - **`HoraTargeted` 该不该改名 `Houjuu`。** 133-2 起这名字的原话理由就是「术语表里没这个词条」，
    而 145 把词条收了 ⇒ **那条理由过期了**（判据 15：决定与它的前提各自过期）。
    **我裁：改。** 纯改名、无行为改动，而 `CONTEXT.md` 是标识符的唯一权威（硬约束 5），
    留着一个「因为当时没词条才起的名字」正是下一个人会问「为什么不叫 Houjuu」的地方。
    ⇒ 并进票 134 的票面（它本来就要读 `SeatTally`），**不单开一票**：单开一张纯改名的票不值一次上下文。
  - **「起家是座位 0」今天在页面上看不出来**（票 121 删了那句话，只剩 `data-anchor` 给机器看；
    名牌写的是转着的自风）。agent 按票面一个字没加，对。
    **我裁：不补。** 记分卡与名牌都写「座位 N」，两处对得上，读者不需要「起家」这个概念就能用这张表；
    真要补是票 121 那条减法的回头账，**该由主人裁，不该由这一票夹带**。⇒ 记档，不开票。
