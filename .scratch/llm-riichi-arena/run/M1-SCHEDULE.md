...
| 35 | **done** | oxkxqskp | ws-c；`?dev=1` 判据只在一处；**加了反向自证**（不带开关时断言开发向 testId 与词汇都不在） | 线上页面把曳光弹等开发向内容藏起来（调度器看线上页面时发现） |
| 37 | **done** | stwryoxq | ws-c；页脚单行、`repoUrl` 单一真源、许可链接派生自 `HEAD`；**顺带给页脚加了会红的闸门** | 站点链回 GitHub 仓库 + 页面上提一句 MIT |
| 36 | **done** | qukkvlrl | ws-a；打码放在**出口**（唯一出海口）而非入口；CI 第十道自带阳性对照；**挖出 34 漏数的第四处流向** | provider 报错原文会把 key 带进牌谱（票 34 挖出的真通道） |
...
| 31 | **done** | mwmowuwy | ws-a；人格/模板/档位三个独立字段；牌谱 v1→v2 只存尾部（Bare 省 69%）；命中率 48%→54% | prompt 降为数据 + system/人格 + 牌谱只存尾部 + 术语表，阻塞于 29b |
...
| 40 | **done** | nwnvqzuk | ws-c；参照系当参数传（`NakiFrame{owner,viewer}`）；他家写「来自**他的**上家」；前缀 30/30 逐字节未变 → 旧缓存不作废 | prompt 里他家副露用观测者参照系，会印出不可能的「吃来自对家」 |
| 39 | **done** | wqlvwwom | ws-b；口径＝局中读 `GameState`、终局读 `Game`（注释里写死的权威）；闸门加 `--to-end`；反向自证按红三次 | 终局点数两个说法 + 末局文案 + `verify-export` 假红 + README 那句已不成立（27-A） |
| 38 | **done** | tyzmkmum | ws-c；横放＝被鸣那张（挑「朝向」这个没被占的维度）＋显式「来自上家/对家/下家」；**票外挖出 prompt 参照系错 → 票 40** | 副露看不出来源与被鸣那张（与 32 同类：数据里有、人看不见） |
| 27 | **done** | ntwmtxms | ws-a；两档各真跑完一整场东风战；spec 四项逐条成立；只报不修立票 39 | | 阻塞于 21、24、25、26、28、29a、29b、30、31、32、33、34、35、36、37 |

## M1 收官

40 号落地后，M1 的 24 张票全部 `ready-for-human`，`./scripts/ci.sh` 全绿，远端 CI 与 Pages 全绿，
站点 https://xerxes-2.github.io/janpo/ 活着。剩下的都是**待主人裁**的事项（见本文件末尾与
`DECISIONS.md` 的待裁索引三批），没有挡住任何东西的技术债。
...
- **32 集成**（调度器）：**我自己把 `docs/images/table.png` 打开看了**（新规矩的第一次执行）：
  座位 1/2/3 各 10/10/11 张斜纹牌背与「手牌 N」逐个对得上，座位 0 明牌，摸切虚线仍分得出，
  场况行末尾的空标签已消失。CI 全绿。

- **31 集成**（调度器）：`README.md` 冲突需人判——31 改的是**旧版开发向 README**里
  「这个仓库是怎么造出来的」一节的两句事实，而 33 已把那节整节删除。取 33 的用户向版本、
  丢掉那两句，并核实 `docs/development.md` 无对应位置需要补。CI 全绿（牌谱已 v2、34 的反向自证仍按红）。
