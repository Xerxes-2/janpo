...
| 88 | 待派（**87 已集成，阻塞解除**） | 87 | 响应动作齐全：吃碰杠、立直两段、荣和自摸、过——**全由引擎的合法动作集驱动** |
...
| 94 | 待派（**95 已集成，阻塞解除**） | — | ToolSearch 档：单步 what-if 工具 + 调用上限 + 延迟与账单的乘数 |
...
| 波 | 内容 | 地盘 |
|---|---|---|
| W1 | 87 **已集成** ∥ 95 **已集成** ∥ 91（ws-b，第二轮在跑） | 三条线互不相交：页面 / Rust+WASM / `web/src/agent`；地盘按下面六条裁定切死 |
