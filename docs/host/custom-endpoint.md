...
| `自定义端点的 baseUrl 读不懂：「localhost:11434」` | 少了 `http://` | 补上协议 |
| `连不上自定义端点 …：浏览器没能把请求发出去` | 端点没起 / 地址写错 / **CORS 没放行** / 公网页面没拿到本地网络授权 | 按 2、3 两节查；`curl` 一下端点确认它活着 |
| `自定义端点 … 回了 404：baseUrl 多半漏了 /v1` | baseUrl 少了路径前缀 | 补 `/v1` |
