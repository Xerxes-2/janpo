...
**一句话**：模型档案里的 provider 选「自定义端点（OpenAI 兼容）」，填一个 **baseUrl**，
...
2. 牌桌页面的**模型档案**编辑处：**provider** 选「自定义端点（OpenAI 兼容）」→ 那里会多出一格
...
配置只落在这台浏览器的 localStorage（键名 `janpo.profiles.0.base_url` 等，票 73 起一份档案一组键），
不经任何后端。自己往 localStorage 里写值的话，`janpo.profiles.0.provider` 要填 **`custom-openai`**；
...
| `自定义端点的 baseUrl 读不懂：「[端点地址已打码]」` | 少了 `http://` | 补上协议（你填的原文不会回显，看档案里那一格） |
...
| `模型超时（… ms 没答完）` | 本地模型太慢 | 把档案里的「超时 (ms)」调大；本地大模型首次加载几十秒很常见 |

每句话末尾还有一个括号，它说的是**这一手重问了几遍**：

- `（重试 2 次仍无结果）`——问了三遍都不行。
- `（没有重试：…，再问一遍还是一样）`——**只问了一遍就兜底了**，因为那个错重试没意义
  （票 47）。鉴权不过、请求本身不合法、模型名不存在这几类属于它：重试发的是同一份请求，
  拿回来的也是同一个答案。**看到这句话不是平台少试了，是它判定多试也白搭。**
  限流（429）、端点 5xx、超时与连不上仍然重试 2 次。
...
- **本地模型慢是常态**：超时默认 240 秒（票 72 从 30 秒改上来的：开着思考预算的模型
  实测单手 17–180 秒），本地 7B 起步的模型第一次加载仍可能更久。
