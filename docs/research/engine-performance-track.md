...
- **第 6 轮给出的那张实现票还没立**：E2（`AgariShape.waits` 向听前置闸）+ E3（`RiichiState.canDeclare`
  单次向听 ≤ 0）+ 两条等价性属性测试 + 2025/2026 重扫验收。草案在 `step-cost-on-replay-path.md` §11，
  可证伪清单在 §10。预验证已做完（引擎重放 2.06×，语义三道闸全过），但**它不是全扫可行性的必需品**
  （32 核不改代码 18 包 ≈42 分钟），做不做由人裁。
