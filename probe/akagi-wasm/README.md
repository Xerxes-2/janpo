...
## 遮住他家的手牌，它出的手变不变（票 92）

票 92 里它消费的是**掩蔽事件流**（`DecisionPackage.history`），而上面那几个脚本量的全是
**完整**的天凤牌谱（四家配牌都摊着）。两者不是同一份输入，因此票 92 开工前先量了一遍：

```sh
node mask-parity.mjs --seats 0,1,2,3 ../../tests/fixtures/paifu/mjai/*.mjson
#   牌谱 111 份、座位 0/1/2/3、决策点 69318 个
#   掩蔽之后被引擎拒掉的行：0
#   两份输入下出的手不同的决策点：0
```

**69,318 个决策点，遮与不遮逐条同一手。** 两件事一起有了答案：掩蔽流喂得进去
（它不会被 13 张 `"?"` 弄崩），以及**它不看他家的手牌**——那不是读 `obs.rs` 的通道表
得来的印象，是量出来的。
...
| `mask-parity.mjs` | **票 92 的地基实验**：遮住他家的手牌，它出的手变不变（见下） |
